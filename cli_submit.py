...
----------------------------------------
Orchestrates full training → inference → validation → packaging flow.
Now includes auto-triggered selftest check before pipeline execution.
"""
...
def make_submission(
    ...
    """Run full V50 pipeline: selftest → train → infer → calibrate → validate → bundle"""
    run_selftest()
